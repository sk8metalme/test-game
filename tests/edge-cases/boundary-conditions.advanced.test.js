...
describe('Advanced Boundary Conditions and Edge Cases', () => {
  ...
  describe('極端な境界条件', () => {
    ...
    test('回転時の境界超過処理', () => {
      ...
      expect(board.canPlacePiece(cellsAfterMove, tetromino.position.x, tetromino.position.y)).toBe(true);
    });
  });
  ...
  describe('データ整合性とエラー復旧', () => {
    ...
    test('無効な位置データの処理', () => {
      ...
      expect(() => {
        ...
        board.canPlacePiece(tetromino.getOccupiedCells(), tetromino.position.x, tetromino.position.y);
      }).toThrow();
    });
  ...
  describe('極端な入力値の処理', () => {
    test('極大・極小値での位置設定', () => {
      ...
      expect(board.canPlacePiece(tetromino.getOccupiedCells(), tetromino.position.x, tetromino.position.y)).toBe(false);
    });
