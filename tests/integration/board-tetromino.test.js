...
describe('Board-Tetromino Integration Tests', () => {
  describe('ピース配置統合テスト', () => {
    ...
    test('T字ピースをボードに正常配置できる', () => {
      ...
      // ピース配置前の状態確認
      expect(board.canPlacePiece(occupiedCells, tetromino.position.x, tetromino.position.y)).toBe(
        true
      );
    ...
    test('境界近くでのピース配置テスト', () => {
      ...
      expect(
        board.canPlacePiece(leftCells, tetrominoLeft.position.x, tetrominoLeft.position.y)
      ).toBe(true);
      ...
      expect(
        board.canPlacePiece(rightCells, tetrominoRight.position.x, tetrominoRight.position.y)
      ).toBe(true);
      ...
      expect(
        board.canPlacePiece(rightOCells, tetrominoRightO.position.x, tetrominoRightO.position.y)
      ).toBe(true);
      ...
      expect(
        board.canPlacePiece(bottomCells, tetrominoBottom.position.x, tetrominoBottom.position.y)
      ).toBe(true);
    });
  });
  ...
  describe('衝突検知統合テスト', () => {
    ...
    test('境界を超える配置の検知', () => {
      ...
      expect(
        board.canPlacePiece(cellsLeft, tetrominoLeft.position.x, tetrominoLeft.position.y)
      ).toBe(false);
      ...
      expect(
        board.canPlacePiece(cellsRight, tetrominoRight.position.x, tetrominoRight.position.y)
      ).toBe(false);
      ...
      expect(
        board.canPlacePiece(cellsBottom, tetrominoBottom.position.x, tetrominoBottom.position.y)
      ).toBe(false);
    });
  });
