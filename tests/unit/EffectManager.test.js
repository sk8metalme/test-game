...
describe('EffectManager', () => {
  ...
  describe('コンストラクタ', () => {
    test('デフォルト設定でEffectManagerを作成できる', () => {
      ...
      expect(manager.effects.size).toBe(5); // line-clear, t-spin, perfect-clear, level-up, game-over
  ...
  describe('エフェクト再生', () => {
    ...
    test('ゲームオーバーエフェクトを再生できる', () => {
      const result = effectManager.playEffect('game-over', {
        position: { x: 400, y: 300 },
        intensity: 1.0,
      });
  ...
  describe('文字列表現', () => {
    test('文字列表現を取得できる', () => {
      ...
      expect(str).toContain('5'); // registered effects
