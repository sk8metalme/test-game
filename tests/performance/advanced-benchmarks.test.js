...
describe('Advanced Performance Benchmarks', () => {
    ...
    test('長時間動作でのメモリリーク検出', () => {
      ...
      for (let batch = 0; batch < operationCount / batchSize; batch++) {
        ...
        if (batch % 10 === 9) {
          ...
          // メモリ増加が35MB以下であることを確認（テスト環境に合わせて調整）
          expect(growth).toBeLessThan(35 * 1024 * 1024);
        }
      }
  ...
  describe('スケーラビリティテスト', () => {
    test('大きなボードサイズでの性能', () => {
      // 通常の2倍サイズのボードをシミュレート
      const largeBoard = new Board();
      
      // 40x20の仮想ボードをシミュレート（通常の2倍の高さ）
      const virtualHeight = 40;
      const virtualGrid = Array(virtualHeight).fill(null)
        .map(() => Array(10).fill(0));
